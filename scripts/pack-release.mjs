...
export async function packReleaseArchive(options = {}) {
  ...
  electronPackage.files = Array.isArray(electronPackage.files)
    ? [...new Set([...electronPackage.files, BRIDGE_MARKER])]
    : [BRIDGE_MARKER];
  ...
  const originalSource = await readFile(electronInstallPath, "utf8");
  const patchedSource = patchedElectronInstallSource(originalSource, electronVersion, bridgedVersion);
  ...
  try {
    ...
    await Promise.all([
      ...
      writeFile(path.join(packedElectronRoot, "install.js"), patchedSource, "utf8"),
