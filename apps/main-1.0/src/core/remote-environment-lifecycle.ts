...
export class RemoteEnvironmentLifecycle {
  ...
  saveEnvironment(input: EnvironmentUpsertInput): SessionEnvironment {
    const previous = input.id ? this.store.getEnvironment(input.id) : null;
    const environment = this.store.upsertEnvironment(input);
    try {
      this.onEnvironmentSaved(environment, input);
    } catch (error) {
      if (previous) this.store.upsertEnvironment(environmentInput(previous));
      else this.store.deleteEnvironment(environment.id);
      throw error;
    }
