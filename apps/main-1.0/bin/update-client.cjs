...
function resolveNpmCommand(options = {}) {
  ...
  const executableName = (options.platform || process.platform) === "win32" ? "npm.cmd" : "npm";
...
async function stageUpdate(manifest, options = {}) {
  ...
  // The updater is loaded from the live package itself. Using that package
  // root avoids a synchronous `npm root -g` lookup, which fails when a GUI
  // launch inherits macOS's minimal PATH.
