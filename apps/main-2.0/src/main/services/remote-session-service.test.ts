...
function createHarness(options: {
  ...
  const setSessionSourceAvailable = vi.fn(async (sessionKey: string, available: boolean) => {
  ...
  const store = {
    ...
    getSessionSourceArtifacts: vi.fn(async () => []),
    isSessionContentFresh: vi.fn(async (sessionKey: string) => !uncachedSessionKeys.has(sessionKey)),
...
describe("RemoteSessionService cloud orchestration", () => {
  ...
  it("checks unavailable cached transcripts with bounded concurrency", async () => {
    const sessions = Array.from({ length: 12 }, (_, index) => localSession({
      sessionKey: `ssh:cached-${index}`,
      rawId: `cached-${index}`,
      filePath: `/tmp/cached-${index}.jsonl`,
      environmentId: "ssh-1",
      environmentKind: "ssh",
      environmentLabel: "SSH",
      sourceAvailable: false,
    ...
    let active = 0;
    let peak = 0;
    vi.mocked(harness.store.isSessionContentFresh).mockImplementation(async () => {
      active += 1;
