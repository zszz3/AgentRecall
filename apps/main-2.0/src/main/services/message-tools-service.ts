...
export class MessageToolsService {
  ...
  async resolve(value: unknown): Promise<{ sessionKey: string; hit: SessionMatchHit }> {
    ...
    const turnId = await this.store.getMessageTurnId(locator.sessionKey, message.index);
    if (!turnId) throw new Error("The message's conversation turn is unavailable. Refresh the session.");
