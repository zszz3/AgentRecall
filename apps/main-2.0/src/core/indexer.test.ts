...
describe("indexer", () => {
  ...
  it("refreshes one Codex title without loading or rewriting any conversation", async () => {
    ...
    try {
      ...
      expect(await store.searchSessions({ query: "Stable Title", limit: 10 })).toHaveLength(1);
      await store.setCustomTitle("codex:first", "My custom title");
      ...
      await syncDefaultSessionsInBatches(store, { loadOptions: { homeDir } });
      expect((await store.getSession("codex:first"))?.originalTitle).toBe("Embedded Title");
      expect((await store.getSession("codex:first"))?.displayTitle).toBe("My custom title");
      expect(upsert).not.toHaveBeenCalled();
    } finally {
      await store.close();
      fs.rmSync(homeDir, { recursive: true, force: true });
