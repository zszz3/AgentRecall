...
describe("remote sync", () => {
  ...
  it("indexes only the effective Codex and Claude branches from SSH summaries", async () => {
    ...
    try {
      ...
      const codexSession = await store.getSession("ssh:ssh-devbox:codex-cli:codex-visible");
      const claudeSession = await store.getSession("ssh:ssh-devbox:claude-cli:claude-visible");
