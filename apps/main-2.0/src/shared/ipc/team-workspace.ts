...
export const teamRequestSchema = z.discriminatedUnion("action", [
  ...
  z.object({ action: z.literal("add-team"), id, name: text, repository }).strict(),
  z.object({ action: z.literal("default-team"), id: id.nullable() }).strict(),
  z.object({ action: z.literal("add-project"), id, name: text, directory, remote: text.optional(), teamId: id.nullable().optional() }).strict(),
