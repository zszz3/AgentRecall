...
- **目录级长期记忆**：在 Memory 页面为主动选择的目录启用相互隔离的 OpenViking 记忆，按需导入历史会话，并为 Codex、Claude Code 和 OpenCode 配置自动召回。
...
- 按 Skill 查看最近的触发记录，直接打开每次触发所在的会话；
- Codex 会话中的触发自动关联，早期没有关联信息的记录会如实标注为"未关联"；
- 所有分析只读取本机已索引的数据。

设计文档见 [docs/eval/](../../docs/eval/README.md)。
