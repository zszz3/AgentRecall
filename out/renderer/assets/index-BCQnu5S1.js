...
function formatCompactNumber(value) {
  ...
  if (safeValue >= 1e9) return `${trimCompactDecimal(safeValue / 1e9)}B`;
  if (safeValue >= 1e6) return `${trimCompactDecimal(safeValue / 1e6)}M`;
  if (safeValue >= 1e3) return `${trimCompactDecimal(safeValue / 1e3)}K`;
...
function readSidebarSections(value) {
...
function serializeSidebarSections(state) {
  return JSON.stringify(state);
}
function toggleSidebarSection(state, section) {
  return { ...state, [section]: !state[section] };
}
const THEME_STORAGE_KEY = "agent-session-search-theme";
function readStoredTheme(value) {
...
function sourceFilters(settings) {
  return [
    ...BASE_SOURCE_FILTERS,
    ...settings?.includeClaudeInternal ? [{ label: "Claude Internal", value: "claude-internal" }] : [],
    ...settings?.includeCodexInternal ? [{ label: "Codex Internal", value: "codex-internal" }] : []
  ];
}
const SORT_OPTIONS = [
...
function App() {
  const [theme, setTheme] = reactExports.useState(() => readInitialTheme());
  const [sidebarSections, setSidebarSections] = reactExports.useState(() => loadInitialSidebarSections());
  const [query, setQuery] = reactExports.useState("");
  const [source, setSource] = reactExports.useState("all");
  const [tag, setTag] = reactExports.useState();
  const [projectPath, setProjectPath] = reactExports.useState();
  const [visibility, setVisibility] = reactExports.useState("default");
  ...
  const [statsPeriod, setStatsPeriod] = reactExports.useState("today");
  const [statsRefreshing, setStatsRefreshing] = reactExports.useState(false);
  const [statsFeedback, setStatsFeedback] = reactExports.useState(null);
  const [quotas, setQuotas] = reactExports.useState(EMPTY_QUOTAS);
  const [quotaLoading, setQuotaLoading] = reactExports.useState(false);
  const [quotaFeedback, setQuotaFeedback] = reactExports.useState(null);
  const [indexStatus, setIndexStatus] = reactExports.useState(null);
  ...
  const [deleteTagName, setDeleteTagName] = reactExports.useState(null);
  const [actionStatus, setActionStatus] = reactExports.useState(null);
  const [refreshFeedback, setRefreshFeedback] = reactExports.useState(null);
  const [settingsOpen, setSettingsOpen] = reactExports.useState(false);
  const [appSettings, setAppSettings] = reactExports.useState(null);
  const [settingsFeedback, setSettingsFeedback] = reactExports.useState(null);
  const [pendingInternal, setPendingInternal] = reactExports.useState({ claude: false, codex: false });
  const loadSeqRef = reactExports.useRef(0);
  const detailLoadSeqRef = reactExports.useRef(0);
  ...
  const load = reactExports.useCallback(async () => {
    ...
    setStats(nextStats);
    setSelectedKey(
      (current) => current && !nextResults.some((session) => session.sessionKey === current) ? null : current
    );
  }, [query, source, tag, projectPath, visibility, sortBy, statsPeriod]);
  const refreshStats = reactExports.useCallback(async () => {
  ...
  const loadQuotas = reactExports.useCallback(async (manual = false) => {
  ...
  reactExports.useEffect(() => {
    const onKeyDown = (event) => {
      ...
      if (event.key === "ArrowDown" || event.key === "ArrowUp") {
        ...
        const nextIndex = currentIndex < 0 ? delta === 1 ? 0 : results.length - 1 : Math.min(results.length - 1, Math.max(0, currentIndex + delta));
  ...
  const visibleSourceFilters = reactExports.useMemo(() => {
  ...
  const selectedProject = reactExports.useMemo(
    () => projects.find((project) => project.path === projectPath) || null,
    [projects, projectPath]
  );
  const searchPlaceholder = projectPath ? `Search within ${selectedProject?.label || "project"}` : tag ? `Search within #${tag}` : "Search titles, first questions, full text, paths, or ids";
  function toggleSidebarSectionById(section) {
  ...
  function beginAddTag(session) {
    setContextMenu(null);
    setDialog({ kind: "tag", session, value: "" });
  }
  async function submitDialog(valueOverride) {
    if (!dialog) return;
  ...
  async function removeTag(session, tagName) {
  ...
  async function updateDefaultTerminal(defaultTerminal) {
  ...
  async function updateSettings(next) {
    ...
    try {
      ...
      if (enablingClaude || enablingCodex) {
        ...
        void window.sessionSearch.refreshIndex().then(async () => {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "app", "data-theme": theme, onClick: () => setContextMenu(null), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "titlebar-drag" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "sidebar", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "brand", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "brand-mark", children: /* @__PURE__ */ jsxRuntimeExports.jsx(Search, { size: 17 }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h1", { children: "Agent Session Search" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Codex and Claude Code" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "refresh-control", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `primary ${indexStatus?.running ? "is-running" : ""}`, onClick: () => void refreshNow(), disabled: indexStatus?.running, children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(RefreshCw, { size: 16 }),
          indexStatus?.running ? "Refreshing Index..." : "Refresh Index"
        ] }),
        refreshFeedback ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `refresh-feedback ${refreshFeedback.kind}`, children: refreshFeedback.message }) : null
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stats-panel", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stats-header", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Usage" }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stats-controls", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "stats-period-toggle", role: "group", "aria-label": "Usage period", children: STATS_PERIOD_OPTIONS.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
        ...
        statsFeedback ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `stats-feedback ${statsFeedback.kind}`, children: statsFeedback.message }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "stats-metrics", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: formatCompactNumber(stats.total.messageCount) }),
            "Messages"
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: formatTokenCount(stats.total.totalTokens) }),
            "Tokens"
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "stats-breakdown", children: stats.bySource.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: SOURCE_LABEL[item.source] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("em", { children: [
            formatCompactNumber(item.messageCount),
            " msg · ",
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarSectionHeader, { title: "Views", expanded: sidebarSections.views, onToggle: () => toggleSidebarSectionById("views") }),
      sidebarSections.views ? /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "nav-group", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: visibility === "default" ? "active" : "", onClick: () => setVisibility("default"), children: "All" }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: visibility === "favorites" ? "active" : "", onClick: () => setVisibility("favorites"), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Star, { size: 14 }),
          "Favorites"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: visibility === "pinned" ? "active" : "", onClick: () => setVisibility("pinned"), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Pin, { size: 14 }),
          "Pinned"
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: visibility === "hidden" ? "active" : "", onClick: () => setVisibility("hidden"), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(EyeOff, { size: 14 }),
          "Hidden"
        ] })
      ] }) : null,
      /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarSectionHeader, { title: "Sources", expanded: sidebarSections.sources, onToggle: () => toggleSidebarSectionById("sources") }),
      sidebarSections.sources ? /* @__PURE__ */ jsxRuntimeExports.jsx("nav", { className: "nav-group", children: visibleSourceFilters.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: source === item.value ? "active" : "", onClick: () => setSource(item.value), children: item.label }, item.label)) }) : null,
      /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarSectionHeader, { title: "Projects", expanded: sidebarSections.projects, onToggle: () => toggleSidebarSectionById("projects") }),
      sidebarSections.projects ? /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "project-list", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: !projectPath ? "active" : "", onClick: () => setProjectPath(void 0), children: "All Projects" }),
        projects.map((project) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            ...
            children: [
              ...
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: project.label }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: project.sessionCount })
            ]
          },
          project.path
        ))
      ] }) : null,
      /* @__PURE__ */ jsxRuntimeExports.jsx(SidebarSectionHeader, { title: "Tags", expanded: sidebarSections.tags, onToggle: () => toggleSidebarSectionById("tags") }),
      sidebarSections.tags ? /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "tag-list", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: !tag ? "active" : "", onClick: () => setTag(void 0), children: "All Tags" }),
        tags.map((tagName) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            className: `tag-list-row ${tag === tagName ? "active" : ""} ${isBranchTag(tagName) ? "branch-tag" : ""}`,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "tag-filter", onClick: () => setTag(tagName), title: `Filter by ${tagName}`, children: [
                isBranchTag(tagName) ? /* @__PURE__ */ jsxRuntimeExports.jsx(GitBranch, { size: 13 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Tag, { size: 13 }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: tagName })
              ] }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "button",
                {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "content", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "toolbar", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "searchbox", children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "kbd-hint", children: "⌘K" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "kbd-hint", title: "Resume selected session in the default terminal", children: "⌘↵" })
        ] }),
        selectedProject ? /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "chip clear", onClick: () => setProjectPath(void 0), title: selectedProject.path, children: [
          selectedProject.label,
          " ×"
        ] }) : null,
        tag ? /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "chip clear", onClick: () => setTag(void 0), children: [
          "#",
          tag,
          " ×"
        ] }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "sort-menu", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Sort" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("select", { value: sortBy, onChange: (event) => setSortBy(event.target.value), children: SORT_OPTIONS.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: option.value, children: option.label }, option.value)) })
        ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "result-count", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          results.length,
          " sessions"
        ] }),
        selected ? /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "selected-path", children: selected.projectPath || selected.rawId }) : null
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "results", children: [
        results.map((session) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          SessionRow,
          {
            session,
        ...
        results.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "empty", children: "No sessions found." }) : null
      ] })
    ] }),
    detail ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      DetailPanel,
      {
        ...
        onResume: () => void runAction("Opening terminal", () => window.sessionSearch.resumeSession(detail.sessionKey), "Resume command sent to terminal."),
        onResumeIterm: () => void runAction("Opening iTerm", () => window.sessionSearch.resumeSessionInIterm(detail.sessionKey), "Resume command sent to iTerm."),
        onCopyResume: () => void runAction("Copying resume command", () => window.sessionSearch.copyResumeCommand(detail.sessionKey), "Resume command copied."),
        onCopyMarkdown: () => void runAction("Copying markdown", () => window.sessionSearch.copyMarkdown(detail.sessionKey), "Markdown copied."),
        onCopyPlain: () => void runAction("Copying plain text", () => window.sessionSearch.copyPlainText(detail.sessionKey), "Plain text copied."),
    ...
    contextMenu ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      ContextMenu,
      {
        ...
        onPin: () => void runAction("Updating pin", () => window.sessionSearch.setPinned(contextMenu.session.sessionKey, !contextMenu.session.pinned), "Pin updated."),
        onHide: () => void runAction(
          "Updating visibility",
          () => window.sessionSearch.setHidden(contextMenu.session.sessionKey, !contextMenu.session.hidden),
          "Visibility updated."
        ),
        onResume: () => void runAction("Opening terminal", () => window.sessionSearch.resumeSession(contextMenu.session.sessionKey), "Resume command sent to terminal."),
        onResumeIterm: () => void runAction("Opening iTerm", () => window.sessionSearch.resumeSessionInIterm(contextMenu.session.sessionKey), "Resume command sent to iTerm."),
        onOpenApp: () => void runAction("Opening native app", () => window.sessionSearch.openNativeApp(contextMenu.session.sessionKey), "Native app opened."),
        onCopyResume: () => void runAction("Copying resume command", () => window.sessionSearch.copyResumeCommand(contextMenu.session.sessionKey), "Resume command copied."),
        onCopyMarkdown: () => void runAction("Copying markdown", () => window.sessionSearch.copyMarkdown(contextMenu.session.sessionKey), "Markdown copied."),
        onCopyPlain: () => void runAction("Copying plain text", () => window.sessionSearch.copyPlainText(contextMenu.session.sessionKey), "Plain text copied."),
        onReveal: () => void runAction("Opening Finder", () => window.sessionSearch.revealSession(contextMenu.session.sessionKey), "Finder opened.")
      }
    ) : null,
    actionStatus ? /* @__PURE__ */ jsxRuntimeExports.jsx(ActionToast, { status: actionStatus }) : null,
    dialog ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      CommandDialog,
      {
        dialog,
        tags,
    ...
    deleteTagName ? /* @__PURE__ */ jsxRuntimeExports.jsx(
      DeleteTagDialog,
      {
...
function QuotaPanel({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "quota-panel", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "quota-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Remaining" }),
        updatedAt ? /* @__PURE__ */ jsxRuntimeExports.jsx("em", { children: updatedAt }) : null
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "quota-refresh", onClick: onRefresh, disabled: loading, title: "Refresh usage limits", "aria-label": "Refresh usage limits", children: /* @__PURE__ */ jsxRuntimeExports.jsx(RefreshCw, { size: 13 }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "quota-list", children: [
      snapshot.providers.map((card) => /* @__PURE__ */ jsxRuntimeExports.jsx(QuotaProviderCard, { card }, card.provider)),
      snapshot.providers.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "quota-empty", children: loading ? "Checking usage limits..." : "Usage limits unavailable." }) : null
    ] }),
    feedback ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `quota-feedback ${feedback.kind}`, children: feedback.message }) : null
  ] });
}
function QuotaProviderCard({ card }) {
  const supported = card.status === "supported" && card.quotas.length > 0;
  const meta = card.plan;
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `quota-card ${card.provider}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "quota-provider-head", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "quota-provider-name", children: card.displayName }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `quota-status ${card.status}`, children: quotaStatusLabel(card.status) })
    ] }),
    meta ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "quota-meta", children: meta }) : null,
    supported ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "quota-windows", children: card.quotas.map((quota) => /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "quota-window", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "quota-window-top", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: quota.label }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
          quota.remainingDisplay,
          " left"
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "quota-track", "aria-hidden": "true", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "quota-fill", style: { width: `${quota.remainingPercent}%` } }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "quota-reset", children: quota.stale ? "stale" : formatQuotaReset(quota.resetsAt) })
    ] }, quota.key)) }) : /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "quota-detail", children: card.detail || "Quota data unavailable." })
  ] });
}
function SidebarSectionHeader({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "section-header", onClick: onToggle, "aria-expanded": expanded, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: title }),
    expanded ? /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronDown, { size: 14 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { size: 14 })
  ] });
}
function SessionRow({
  session,
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "article",
    {
      ...
      onContextMenu,
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "session-main", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "session-title", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: `source-dot ${session.source.startsWith("claude") ? "claude" : "codex"}` }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: /* @__PURE__ */ jsxRuntimeExports.jsx(Star, { size: 14, fill: session.favorited ? "currentColor" : "none" })
              }
            ),
            session.pinned ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pin, { size: 14 }) : null,
            session.hidden ? /* @__PURE__ */ jsxRuntimeExports.jsx(EyeOff, { size: 14 }) : null,
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "session-name", children: session.displayTitle }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "session-meta", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: `source-badge ${session.source.startsWith("claude") ? "claude" : "codex"}`, children: [
              session.source.startsWith("claude") ? /* @__PURE__ */ jsxRuntimeExports.jsx(CodeXml, { size: 13 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Terminal, { size: 13 }),
              SOURCE_LABEL[session.source]
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: session.projectPath || "No project path" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatRelativeTime(session.timestamp) }),
            /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { children: [
          ...
          session.matchSnippet ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "snippet", children: session.matchSnippet }) : null
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "row-tags", children: session.tags.slice(0, 3).map((tagName) => /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: isBranchTag(tagName) ? "branch-tag" : void 0, children: [
          "#",
          tagName
        ] }, tagName)) })
      ]
...
function DetailPanel({
  ...
  const matchIndex = query ? messages.findIndex((message) => message.content.toLowerCase().includes(query.toLowerCase())) : -1;
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-backdrop", onClick: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("aside", { className: "detail", onClick: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-header", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `source-badge ${session.source.startsWith("claude") ? "claude" : "codex"}`, children: SOURCE_LABEL[session.source] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-title-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("h2", { children: session.displayTitle }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "title-edit-button detail-title-edit", onClick: onRenameTitle, "aria-label": "Rename session", title: "Rename session", children: /* @__PURE__ */ jsxRuntimeExports.jsx(PenLine, { size: 14 }) })
        ] }),
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-header-actions", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            ...
            "aria-label": session.favorited ? "Remove from favorites" : "Add to favorites",
            title: session.favorited ? "Remove from favorites" : "Add to favorites",
            children: /* @__PURE__ */ jsxRuntimeExports.jsx(Star, { size: 17, fill: session.favorited ? "currentColor" : "none" })
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "icon-button", onClick: onClose, "aria-label": "Close", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 17 }) })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-actions", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onResume, disabled: actionRunning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 15 }),
        " Resume"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onResumeIterm, disabled: actionRunning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Terminal, { size: 15 }),
        " iTerm"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onRename, disabled: actionRunning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Clipboard, { size: 15 }),
        " Rename"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onAddTag, disabled: actionRunning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Tag, { size: 15 }),
        " Add Tag"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onCopyResume, disabled: actionRunning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 15 }),
        " Copy Cmd"
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onCopyMarkdown, disabled: actionRunning, children: "Markdown" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onCopyPlain, disabled: actionRunning, children: "Plain Text" }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onReveal, disabled: actionRunning, children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(FolderOpen, { size: 15 }),
        " Finder"
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "detail-tags", children: session.tags.map((tagName) => /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: `chip ${isBranchTag(tagName) ? "branch-tag" : ""}`, onClick: () => onRemoveTag(tagName), children: [
      "#",
      tagName,
      " ×"
    ] }, tagName)) }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "detail-body", ref: bodyRef, children: [
      context.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "matched", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Matched Context" }),
        context.map((message) => /* @__PURE__ */ jsxRuntimeExports.jsx(MessageBlock, { message, query }, message.index))
      ] }) : null,
      /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "conversation", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Full Conversation" }),
        loading ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "loading-state", children: "Loading conversation..." }) : null,
        !loading && messages.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "loading-state", children: "No visible messages indexed for this session." }) : null,
        messages.map((message) => /* @__PURE__ */ jsxRuntimeExports.jsx(MessageBlock, { message, query }, message.index)),
        !loading && messages.length < session.messageCount ? /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: "show-more", onClick: onShowMore, children: [
          "Show ",
          Math.min(MESSAGE_PAGE_SIZE, session.messageCount - messages.length),
          " more messages"
        ] }) : null
      ] })
    ] })
...
function MessageBlock({ message, query }) {
  const content = reactExports.useMemo(() => {
    const text = message.content.length > 3e3 ? `${message.content.slice(0, 3e3)}

...(truncated)` : message.content;
    if (!query) return text;
    return text;
  }, [message.content, query]);
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: `message ${message.role}`, children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "message-head", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: message.role === "user" ? "User" : "Assistant" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatMessageTime(message.timestamp) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("pre", { children: content })
  ] });
}
function ActionToast({ status }) {
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `action-toast ${status.kind}`, role: "status", "aria-live": "polite", children: status.message });
}
function ContextMenu({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "context-menu", style: { left: state.x, top: state.y }, onClick: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onRename, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Clipboard, { size: 14 }),
      " Rename"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onAddTag, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Tag, { size: 14 }),
      " Add Tag"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onFavorite, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Star, { size: 14, fill: state.session.favorited ? "currentColor" : "none" }),
      " ",
      state.session.favorited ? "Unfavorite" : "Favorite"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onPin, children: [
      state.session.pinned ? /* @__PURE__ */ jsxRuntimeExports.jsx(PinOff, { size: 14 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Pin, { size: 14 }),
      " ",
      state.session.pinned ? "Unpin" : "Pin"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onHide, children: [
      state.session.hidden ? /* @__PURE__ */ jsxRuntimeExports.jsx(Eye, { size: 14 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Archive, { size: 14 }),
      " ",
      state.session.hidden ? "Unhide" : "Hide"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("hr", {}),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onResume, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 14 }),
      " Resume in Terminal"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onResumeIterm, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Terminal, { size: 14 }),
      " Resume in iTerm"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onOpenApp, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(AppWindow, { size: 14 }),
      " Open App"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onCopyResume, children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Copy, { size: 14 }),
      " Copy Resume Cmd"
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onCopyMarkdown, children: "Copy Markdown" }),
    /* @__PURE__ */ jsxRuntimeExports.jsx("button", { onClick: onCopyPlain, children: "Copy Plain Text" }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { onClick: onReveal, children: [
...
function SettingsDialog({
  ...
  const [activeSection, setActiveSection] = reactExports.useState("terminal");
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dialog-backdrop", onMouseDown: onClose, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "command-dialog settings-dialog", onMouseDown: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Settings" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "icon-button", onClick: onClose, "aria-label": "Close", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-shell", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("nav", { className: "settings-sidebar", "aria-label": "Settings sections", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: activeSection === "terminal" ? "active" : "", onClick: () => setActiveSection("terminal"), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Terminal, { size: 15 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Default terminal" })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { className: activeSection === "sources" ? "active" : "", onClick: () => setActiveSection("sources"), children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Folder, { size: 15 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Personal sources" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-content", children: [
        activeSection === "terminal" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "settings-pane", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "settings-pane-head", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Default terminal" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Choose which terminal app Resume and the list shortcut use to reopen a session." })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-field", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-field-text", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "settings-field-title", children: "Terminal app" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "settings-field-sub", children: "Applies to Resume and the keyboard shortcut." })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "select",
              {
                ...
                onChange: (event) => onDefaultTerminalChange(event.target.value),
                children: DEFAULT_TERMINAL_OPTIONS.map((option) => /* @__PURE__ */ jsxRuntimeExports.jsx("option", { value: option.value, children: option.label }, option.value))
              }
            )
          ] })
        ] }) : null,
        activeSection === "sources" ? /* @__PURE__ */ jsxRuntimeExports.jsxs("section", { className: "settings-pane", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "settings-pane-head", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("h3", { children: "Personal sources" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("p", { children: "Internal sessions stay separate from the normal Claude and Codex filters." })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "settings-field settings-toggle", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-field-text", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "settings-field-title", children: "Include ~/.claude-internal" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "settings-field-sub", children: "Adds a separate Claude Internal source filter." })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("label", { className: "settings-field settings-toggle", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "settings-field-text", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "settings-field-title", children: "Include ~/.codex-internal" }),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "settings-field-sub", children: "Adds a separate Codex Internal source filter." })
            ] }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "input",
              {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `settings-feedback ${feedback?.kind ?? ""}`, "aria-live": "polite", children: feedback?.message ?? "" })
  ] }) });
}
function DeleteTagDialog({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dialog-backdrop", onMouseDown: onCancel, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "command-dialog", onMouseDown: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog-title", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: "Delete Tag" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "icon-button", onClick: onCancel, "aria-label": "Close", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "dialog-copy", children: [
      "Delete ",
      /* @__PURE__ */ jsxRuntimeExports.jsxs("strong", { children: [
        "#",
        tagName
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog-actions", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "danger-action", onClick: onConfirm, children: "Delete" })
    ] })
  ] }) });
}
function CommandDialog({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "dialog-backdrop", onMouseDown: onCancel, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "form",
    {
      ...
      children: [
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog-title", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: dialog.kind === "rename" ? "Rename Session" : "Add Tag" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", className: "icon-button", onClick: onCancel, "aria-label": "Close", children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "input",
        ...
        matchingTags.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "tag-suggestions", children: matchingTags.map((tagName) => /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", onClick: () => onSubmit(tagName), children: [
          "#",
          tagName
        ] }, tagName)) }) : null,
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "dialog-actions", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "button", onClick: onCancel, children: "Cancel" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { type: "submit", className: "primary-action", children: "Save" })
        ] })
      ]
    }
  ) });
