...
- 支持从 Terminal、iTerm、Ghostty、WezTerm 或 Warp 恢复会话；如果检测到本地会话已经打开，Resume 会优先前置现有终端窗口/标签页，而不是重复启动新的 `codex resume` / `claude --resume`。
- 支持复制 resume 命令，或导出 Markdown。
- 支持本地 Claude Code / Codex / CodeBuddy 会话互相迁移，并在迁移后打开目标会话。
...
- 支持 Skill 使用统计：Claude Code 通过 PostToolUse hook 记录，Codex 从本地 session 中推断读取过的 `SKILL.md`；统计索引写入本地 SQLite，应用启动后、打开 Skills 面板时和点击刷新按钮时都会增量更新。
- 支持 Codex / Claude Code API Provider 切换，内置 CodexZH、DeepSeek、GLM、LongCat、Kimi、MiMo 等常用预设，也支持自定义 base URL、model 和 API key。
- 支持 AI 会话摘要：用自定义 API Provider（兼容 OpenAI Chat / Anthropic，含 GLM 等 coding plan）为会话生成可搜索的「问题 + 方案」摘要并入全文索引；支持单条、批量与自动补全。
- 支持 AI 找会话：右上角 AI 助手按钮打开对话框，用自然语言描述即可让 AI 检索历史并返回可点击的会话卡片。
- 支持 MCP server：以只读方式把历史会话开放给 Claude Code / Codex / CodeBuddy，让它们在对话里直接检索你的历史会话，设置里一键启用。
