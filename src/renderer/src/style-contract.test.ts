...
describe("stylesheet theme contract", () => {
  ...
  it("keeps the 1.0 search, time filter, and core actions aligned", () => {
    const coreToolbar = stylesheet.match(/\.core-toolbar\s*\{[^}]*\}/)?.[0] ?? "";
    ...
    expect(coreToolbar).toMatch(/grid-template-columns:\s*minmax\(220px,\s*1fr\)\s+auto\s+auto/);
    ...
    expect(appSource).toContain('setInfoSection("diagnostics")');
    expect(appSource).not.toContain('className="live-filter"');
    expect(appSource).not.toContain('className="scope-filter"');
  });
