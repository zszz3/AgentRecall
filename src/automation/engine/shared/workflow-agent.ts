...
export function buildWorkflowAgentPrompt({ workflowId, objective }: WorkflowAgentPromptInput): string {
  ...
  return [
    ...
    '- When the user asks for a document or file deliverable, declare artifact metadata on the content output field, for example {"key":"answer_markdown","valueType":"string","artifact":{"format":"markdown","fileName":"report.md"}}. The runtime materializes this content under the current Run output directory.',
    "- Use valueType=file only when the node itself creates a file through a runtime tool. In that case outputs must contain the exact relative file path inside the current Run output directory; the runtime rejects missing or out-of-scope paths.",
    ...
    '- A final summary node must consume the standard upstreamOutputs JSON result packets and produce answer_markdown as the complete user-facing result. When that answer is a requested document, declare artifact={"format":"markdown","fileName":"report.md"}.',
