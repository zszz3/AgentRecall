# 1.0 Core Boundary 接线清单

本分支只交付独立模块和小型注册接口，不修改主进程启动架构。以下步骤应在父会话 A 的 Core Boundary 合入后完成；原生更新与旧 npm 更新器不得同时注册。

## 父会话 A：主进程与 Core API

### 原生更新

1. 用 `registerElectronUpdater()` 替换现有 `AppUpdateService` 注册，保留 `CoreSettings.autoCheckUpdates` 作为用户偏好来源。
...
现有 Core API 仍使用旧的 `AppUpdateStatus` / `AppUpdateInstallResult`。接线时应一次性改为 `NativeUpdateState` 与显式动作，或在主进程放置临时适配器；不要让 renderer 同时看到两套更新状态机。
...
1. 先合入父会话 A 的 Core Boundary，再重放父会话 B 的体验层，最后接入本分支的注册模块。
