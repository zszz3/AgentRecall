...
| WorkBuddy | Optional in settings; reads local sessions from `~/.workbuddy/projects/<project>/<session>.jsonl` and subagents from `<project>/<parent>/subagents/<child>.jsonl` as a read-only source |
...
| ZCode | Optional in settings; reads `~/.zcode/cli/db/db.sqlite`, including tool records and token statistics; a single local session can be permanently deleted after explicit confirmation; supports local migration to and from other agents |
...
Startup and index refresh skip unchanged local session files. Codex title changes update metadata separately; they do not trigger a reread of every conversation. Content updates retain unchanged records and remove content discarded by a rollback. Initial imports, truncated files, or index-format upgrades may still require rereading the affected session. Indexing never modifies the original session files.

CodeBuddy CLI, WorkBuddy, CodeWiz, TClaude, TCodex, OpenClaw, Hermes, OpenCode, ZCode, Cursor Agent, Trae, and Qoder are off by default and can be selected from Settings -> Optional sources. Once enabled, they support local indexing, search, details, and source filtering. WorkBuddy additionally exposes messages, tool traces, titles, and token usage for viewing and export, but its initial integration does not support live tracking, Resume, migration, sync, opening WorkBuddy, or deleting the original transcript. Because TClaude / TCodex share the Claude Code / Codex formats, they additionally support Resume and one-click launch (invoking the `tclaude` / `tcodex` commands respectively). WSL environments support Claude Code, Codex, TClaude, TCodex, and CodeBuddy discovery, details, search, Resume, and manual cloud backup when the corresponding source is enabled. WSL migration accepts an explicit Linux project directory or an explicit no-project-path choice; CodeWiz's shared database is not offered as a WSL/SSH migration target. The watcher falls back to polling when WSL restarts or no event tool is installed, and reports the actual mode while it retries recovery. ZCode also includes local tool-call records and time-ranged token statistics; sessions from other agents can be migrated into ZCode's task list (visible after restarting ZCode), ZCode sessions can also migrate to Claude Code, Codex, CodeBuddy, CodeWiz, Cursor Agent, and enabled TClaude, TCodex, or DeepSeek Harness targets. Enable ZCode, refresh the index, open a session, and select **Migrate to**. Migration creates a new target session and leaves the original ZCode session unchanged; desktop migration includes indexed descendants and their parent relationships. It transfers user and assistant text, not ZCode tool configuration or replayable tool calls. ZCode sessions still do not support Resume, SSH, remote sync, opening ZCode, or quota lookup. Deleting a ZCode session removes only the explicitly selected session and its related records, never the shared database file. For the other sources, Resume, SSH remote sync, and provider-specific usage stats are intentionally separate follow-up work. Trae and Qoder also support open-state detection.

Before migrating into ZCode, create a session in ZCode to initialize its local database and task list. A missing or incompatible task list causes migration to fail rather than leave an invisible new session. A directly selected subagent becomes a standalone task; descendants migrated with their parent remain attached to the new parent. Enable **Settings → Optional sources → ZCode** to search the migrated sessions in AgentRecall.
