...
| ZCode | Optional in settings; reads `~/.zcode/cli/db/db.sqlite`, including tool records and token statistics; a single local session can be permanently deleted after explicit confirmation; also available as a migration target |
