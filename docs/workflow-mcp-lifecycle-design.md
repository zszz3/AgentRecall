...
1. 节点执行开始前创建带 executionId 和 attempt 的持久化 completion ledger；executionId 由宿主注入 MCP 进程，不作为模型可修改的工具参数。
2. Bridge 校验托管权限、workflowId、runId、nodeId、executionId 和输出结构，并将候选 output 原子写入 ledger。
3. 同一 execution 内按规范化 output digest 保证重复提交幂等；新候选会显式 supersede 仍待消费的旧候选，旧 Runtime 不能向新的 execution 提交。
4. one-shot 执行器按 executionId 读取并消费 submission；interactive Conversation Manager 读取 submission 生成带 submissionId 的 completion proposal。
5. Reviewer、Hook 或用户确认通过后，Workflow 执行器才把结果写入权威 `workerOutputs`；拒绝的 interactive submission 会记录为 rejected 并继续同一会话。

tool call/result 事件仍完整进入会话时间线，但只用于展示与审计，不能再作为状态机的机器数据来源。没有 Workflow MCP 的 API Runtime 继续使用 assistant 结构化文本兜底；支持 MCP 的 Runtime 统一使用 submission ledger。
...
Workflow 草稿会话还必须转发 `approval_request` 和 `approval_response`。待处理请求使用现有 Runtime Approval Broker，并在消息时间线中显示“允许一次”和“拒绝”操作；停止回复、重置草稿、切换底层会话、超时和删除 Workflow 时，必须取消对应 owner 的挂起审批。应用重启后内存中的 Broker 请求无法继续，因此持久化但未解决的审批恢复为 `expired`，不能显示成仍可点击的请求。

## 结构化结果与展示摘要边界

工具事件不再同时承担机器消费和界面展示。`workflow_node_complete` 的调用参数仍为审计需要完整保留；普通工具参数和结果可使用受限摘要，避免会话时间线被大段内容占满。状态机只读取 completion ledger，因此渲染摘要、历史恢复和消息排序都不会改变节点结果。

### 结构化输出与文件产物

completion ledger 保存的是节点结构化结果，不等同于用户可见文件。Workflow V2 使用以下规则把二者连接起来：

- `outputFields[].artifact` 明确声明内容型产物，支持 `markdown`、`text`、`json`、`html` 和 `csv`，可指定安全的文件名；Runtime 在节点结果被接受时原子写入当前 Run 的 output 目录。
- 为兼容已有工作流，字段名以 `_markdown` 结尾的字符串输出按 Markdown 产物处理，无需重新生成 Workflow 定义。
- `valueType: file` 表示节点已经通过 Runtime 工具创建文件，结构化输出保存相对路径；Runtime 只接受当前 Run output 目录内真实存在的文件。
- one-shot 节点在任务创建前获得且仅获得当前 Run output 根目录的写入权限；interactive 节点沿用相同边界，确认后再物化内容型产物。
- `workflow_outputs_list` 仍从物理 output 目录读取，因此列表展示、MCP 查询和本地文件保持同一事实来源。

节点完成读取顺序为：当前 execution 的持久化 submission；仅当 Runtime 没有提交 submission 时，才使用 assistant 最终文本兼容不支持 MCP 的 API Runtime。展示层可以折叠或截断历史工具事件，因为该内容不再参与状态转换。
...
- 草稿审批测试：请求和响应进入同一消息时间线，停止与恢复不会留下可操作的僵尸审批。
- 长结构化结果测试：超过普通工具摘要上限且包含多行 Markdown 的 `workflow_node_complete` 参数仍能完整 JSON 解析。
...
- Codex 0.145 会在模型支持 `tool_search` 时延迟暴露 MCP namespace 工具。原生 Responses 渠道保留该发现机制；第三方 Chat Completions 渠道的兼容路由明确关闭搜索能力，使当前 Workflow scope 内的少量 MCP 工具在首轮请求中直接可见，避免依赖模型主动调用模拟的 `tool_search`。兼容路由仍能转换搜索调用与搜索结果，供显式携带该协议的请求使用。
- Workflow 草稿会话已接入 Runtime Approval Broker 事件：真正需要确认的调用会显示审批卡片，生命周期结束时取消挂起请求，重启恢复时将旧请求标记为过期。
- Codex 事件归一化对 `workflow_node_complete` 保留完整参数，结构化节点结果不再经过 600 字符的展示摘要后再交给执行器解析。
- `workflow_node_complete` 已改为直接写入独立的 durable submission ledger；one-shot 与 interactive 节点按 executionId 消费，工具历史仅用于审计，确认后的结果仍由原有 Workflow checkpoint 写入权威 `workerOutputs`。
