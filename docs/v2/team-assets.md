...
重复安装相同配置版本可补齐缺失项或复用相同内容。当前不提供直接的整组更新：同名配置的新版本或其他来源会明确提示冲突，需要先卸载原配置再选择安装；共享 Skill 的新内容也不能静默影响其他配置。`preview` 显示配置本身的版本冲突和每个 Skill 的目标状态。
...
这份记录属于项目的本地管理状态；CLI 不自动提交记录或修改 `.gitignore`。整组版本更新、Rules/Docs/MCP/Agent 模板仍需后续实现。
