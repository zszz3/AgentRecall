...
资产按 Git 提交版本固定，支持用户主动查看差异、更新和回滚，但暂不提供分支订阅、PR 贡献、差异合并或后台自动更新。Rules、Docs、MCP/Agent 配置组合仍在计划中；桌面已有团队入口。现有桌面 Skill 库负责用户级管理；本次新增项目级复制和归属检查，未把桌面扫描器或数据库引入 CLI。
