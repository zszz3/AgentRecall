...
设置以团队仓库、默认团队和项目绑定的紧凑列表呈现；点击添加才展开表单。桌面新增团队只要求仓库 URL，新增项目只要求目录；名称自动识别、内部 ID 自动生成，名称与 Git 远端仍可选填。设为默认与新增团队在同一次配置写入中提交。旧配置格式和 CLI 显式 ID 契约保持不变，不自动开启、同步或上传。

本轮参考 Tencent/teamai-cli 的 `src/init.ts` 与 `docs/designs/dashboard-unified.zh-CN.md`，读取的 main 为 `7c834ce428656e6a2cf6d62bdbf4900c3db12abb`；借鉴仓库地址与项目范围的接入流程，不宣称 TeamAI 提供同样的可写设置界面。V2 相关测试 32 项、CLI 40 项通过；最新跨平台结果以 PR 当前提交为准。
