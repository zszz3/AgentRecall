...
V2 各功能页顶部区分「本地 / 团队」。默认关闭的团队功能在「设置 → 团队」中启用，并管理项目与资产仓库；在「Skills → 团队」手动同步、预览并安装 Skill 和工作配置。其他页面的团队能力尚未开放。开关、项目归属及版本管理与独立 CLI 共用；不会自动上传 Session。配置、确认与失败恢复见[团队工作区指南](team-workspace.md)。
