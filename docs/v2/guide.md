...
在已索引会话的详情中，用户和助手消息下方提供“收藏”和“复制消息链接”：

- 点击“收藏”后，在详情顶部的“重要消息”列表中点击摘要即可跳回原文，取消收藏不会影响整段会话的收藏状态。
- 消息收藏保存在当前版本的本机数据中，关闭应用后仍然保留，目前不随云端会话同步。
- 第一次复制链接时会为当前应用注册消息链接入口。将链接放入支持自定义链接的笔记应用后，可在同一台电脑打开对应消息。V1 与 V2 使用各自的链接入口，链接不会上传消息，也不能让没有该会话的其他设备读取内容。
- 如果原消息的位置随重新索引变化，应用会尝试根据内容重新定位；消息已修改、删除或无法唯一定位时会提示，而不会直接打开另一条消息。
- 云端只读预览中的消息需先恢复并索引，才能使用本机收藏和定位链接。

选择详情顶部的“导出脱敏预览”，可导出 Markdown 或纯文本：

1. 应用在本机检查完整导出文本，包括会话信息与工具轨迹，自动标出可能的密钥、邮箱和路径。
2. 逐项决定是否替换，并编辑替换后的文字。自动选择的内容也可以取消。
3. 可输入其他敏感内容，点击“添加全部相同内容”添加精确匹配。与已有匹配重叠的内容不会重复添加。
4. 右侧默认显示替换后的完整结果，也可切换到带高亮的原文进行核对。
5. 点击“保存已预览的导出”选择保存位置。导出的内容与此次预览一致，即使会话随后新增消息也不会混入未经检查的内容。

自动识别仅作辅助，不保证发现所有敏感信息。附件文件内容不会被扫描，原会话、原附件和已有导出入口均不会修改。预览支持不超过 8 MB 的导出文本，10 分钟后过期，可关闭后重新打开；此入口暂不提供 JSON 导出。
