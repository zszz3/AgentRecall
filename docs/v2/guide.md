...
本地 ZCode 会话支持迁移到 Claude Code、Codex 等 Agent，也可将其他 Agent 的会话迁入 ZCode。操作入口、初始化要求、子代理行为和迁移范围见 [ZCode 迁移说明](../v1/guide.md#迁移会话)。迁入 ZCode 后重启 ZCode，在任务列表中继续。
...
V2 各功能页顶部区分「本地 / 团队」。默认关闭的团队功能在「设置 → 团队」中启用，并连接资产仓库；进入「团队」范围后先选团队、再选项目，在项目的 Skills 页手动同步、预览并安装 Skill 和工作配置。其他页面的团队能力尚未开放。开关、项目归属及版本管理与独立 CLI 共用；不会自动上传 Session。配置、确认与失败恢复见[团队工作区指南](team-workspace.md)。
